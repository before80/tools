...
URIs with an "http" or "https" scheme are normalized and compared according to the methods defined in [Section 6](https://www.rfc-editor.org/rfc/rfc3986#section-6) of [[URI](https://www.rfc-editor.org/rfc/rfc9110.html#URI)], using the defaults described above for each scheme.
...
All [1xx (Informational)](https://www.rfc-editor.org/rfc/rfc9110.html#status.1xx), [204 (No Content)](https://www.rfc-editor.org/rfc/rfc9110.html#status.204), and [304 (Not Modified)](https://www.rfc-editor.org/rfc/rfc9110.html#status.304) responses do not include content.
...
A sender that intends to generate one or more trailer fields in a message **SHOULD** generate a [Trailer](https://www.rfc-editor.org/rfc/rfc9110.html#field.trailer) header field in the header section of that message to indicate which fields might be present in the trailers.

If an intermediary discards the trailer section in transit, the [Trailer](https://www.rfc-editor.org/rfc/rfc9110.html#field.trailer) field could provide a hint of what metadata was lost, though there is no guarantee that a sender of Trailer will always follow through by sending the named fields.
...
In HTTP/2 [[HTTP/2](https://www.rfc-editor.org/rfc/rfc9110.html#HTTP2)] and HTTP/3 [[HTTP/3](https://www.rfc-editor.org/rfc/rfc9110.html#HTTP3)], the Host header field is, in some cases, supplanted by the ":authority" pseudo-header field of a request's control data.
...
If the client has a cache [[CACHING](https://www.rfc-editor.org/rfc/rfc9110.html#CACHING)] and the request can be satisfied by it, then the request is usually directed there first.
...
The [421 (Misdirected Request)](https://www.rfc-editor.org/rfc/rfc9110.html#status.421) status code in a response indicates that the origin server has rejected the request because it appears to have been misdirected ([Section 15.5.20](https://www.rfc-editor.org/rfc/rfc9110.html#status.421)).
...
- Proxy-Connection ([Appendix C.2.2](https://www.rfc-editor.org/rfc/rfc9112#appendix-C.2.2) of [[HTTP/1.1](https://www.rfc-editor.org/rfc/rfc9110.html#HTTP11)])
- Keep-Alive ([Section 19.7.1](https://www.rfc-editor.org/rfc/rfc2068#section-19.7.1) of [[RFC2068](https://www.rfc-editor.org/rfc/rfc9110.html#RFC2068)])
- TE ([Section 10.1.4](https://www.rfc-editor.org/rfc/rfc9110.html#field.te))
- Transfer-Encoding ([Section 6.1](https://www.rfc-editor.org/rfc/rfc9112#section-6.1) of [[HTTP/1.1](https://www.rfc-editor.org/rfc/rfc9110.html#HTTP11)])
- Upgrade ([Section 7.8](https://www.rfc-editor.org/rfc/rfc9110.html#field.upgrade))
...
A server that sends a [426 (Upgrade Required)](https://www.rfc-editor.org/rfc/rfc9110.html#status.426) response **MUST** send an Upgrade header field to indicate the acceptable protocols, in order of descending preference.
...
A sender of Upgrade **MUST** also send an "Upgrade" connection option in the [Connection](https://www.rfc-editor.org/rfc/rfc9110.html#field.connection) header field ([Section 7.6.1](https://www.rfc-editor.org/rfc/rfc9110.html#field.connection)) to inform intermediaries not to forward this field. A server that receives an Upgrade header field in an HTTP/1.0 request **MUST** ignore that Upgrade field.
...
The data type of the representation data is determined via the header fields [Content-Type](https://www.rfc-editor.org/rfc/rfc9110.html#field.content-type) and [Content-Encoding](https://www.rfc-editor.org/rfc/rfc9110.html#field.content-encoding). These define a two-layer, ordered encoding model:
...
Media types are defined in [Section 8.3.1](https://www.rfc-editor.org/rfc/rfc9110.html#media.type). An example of the field is
...
Media types ought to be registered with IANA according to the procedures defined in [[BCP13](https://www.rfc-editor.org/rfc/rfc9110.html#BCP13)].
...
Charset names ought to be registered in the IANA "Character Sets" registry (<https://www.iana.org/assignments/character-sets>) according to the procedures defined in [Section 2](https://www.rfc-editor.org/rfc/rfc2978#section-2) of [[RFC2978](https://www.rfc-editor.org/rfc/rfc9110.html#RFC2978)].
...
An origin server **MAY** respond with a status code of [415 (Unsupported Media Type)](https://www.rfc-editor.org/rfc/rfc9110.html#status.415) if a representation in the request message has a content coding that is not acceptable.
...
All content codings are case-insensitive and ought to be registered within the "HTTP Content Coding Registry", as described in [Section 16.6](https://www.rfc-editor.org/rfc/rfc9110.html#content.coding.extensibility)

Content-coding values are used in the [Accept-Encoding](https://www.rfc-editor.org/rfc/rfc9110.html#field.accept-encoding) ([Section 12.5.3](https://www.rfc-editor.org/rfc/rfc9110.html#field.accept-encoding)) and [Content-Encoding](https://www.rfc-editor.org/rfc/rfc9110.html#field.content-encoding) ([Section 8.4](https://www.rfc-editor.org/rfc/rfc9110.html#field.content-encoding)) header fields.
...
The "compress" coding is an adaptive Lempel-Ziv-Welch (LZW) coding [[Welch](https://www.rfc-editor.org/rfc/rfc9110.html#Welch)] that is commonly produced by the UNIX file compression program "compress". A recipient **SHOULD** consider "x-compress" to be equivalent to "compress".
...
The "deflate" coding is a "zlib" data format [[RFC1950](https://www.rfc-editor.org/rfc/rfc9110.html#RFC1950)] containing a "deflate" compressed data stream [[RFC1951](https://www.rfc-editor.org/rfc/rfc9110.html#RFC1951)] that uses a combination of the Lempel-Ziv (LZ77) compression algorithm and Huffman coding.
...
The "gzip" coding is an LZ77 coding with a 32-bit Cyclic Redundancy Check (CRC) that is commonly produced by the gzip file compression program [[RFC1952](https://www.rfc-editor.org/rfc/rfc9110.html#RFC1952)]. A recipient **SHOULD** consider "x-gzip" to be equivalent to "gzip".
...
A language tag, as defined in [[RFC5646](https://www.rfc-editor.org/rfc/rfc9110.html#RFC5646)], identifies a natural language spoken, written, or otherwise conveyed by human beings for communication of information to other human beings. Computer languages are explicitly excluded.

HTTP uses language tags within the [Accept-Language](https://www.rfc-editor.org/rfc/rfc9110.html#field.accept-language) and [Content-Language](https://www.rfc-editor.org/rfc/rfc9110.html#field.content-language) header fields. [Accept-Language](https://www.rfc-editor.org/rfc/rfc9110.html#field.accept-language) uses the broader language-range production defined in [Section 12.5.4](https://www.rfc-editor.org/rfc/rfc9110.html#field.accept-language), whereas [Content-Language](https://www.rfc-editor.org/rfc/rfc9110.html#field.content-language) uses the language-tag production defined below.
...
See [[RFC5646](https://www.rfc-editor.org/rfc/rfc9110.html#RFC5646)] for further information.
...
In a successful response to a state-changing request, validator fields describe the new representation that has replaced the prior [selected representation](https://www.rfc-editor.org/rfc/rfc9110.html#selected.representation) as a result of processing the request.

For example, an ETag field in a [201 (Created)](https://www.rfc-editor.org/rfc/rfc9110.html#status.201) response communicates the entity tag of the newly created resource's representation, so that the entity tag can be used as a validator in later conditional requests to prevent the "lost update" problem.
...
A "strong validator" is representation metadata that changes value whenever a change occurs to the representation data that would be observable in the content of a [200 (OK)](https://www.rfc-editor.org/rfc/rfc9110.html#status.200) response to GET.

A strong validator might change for reasons other than a change to the representation data, such as when a semantically significant part of the representation metadata is changed (e.g., [Content-Type](https://www.rfc-editor.org/rfc/rfc9110.html#field.content-type)), but it is in the best interests of the origin server to only change the value when it is necessary to invalidate the stored responses held by remote caches and authoring tools.
...
The "Last-Modified" header field in a response provides a timestamp indicating the date and time at which the origin server believes the [selected representation](https://www.rfc-editor.org/rfc/rfc9110.html#selected.representation) was last modified, as determined at the conclusion of handling the request.
...
A sender **MAY** send the ETag field in a trailer section (see [Section 6.5](https://www.rfc-editor.org/rfc/rfc9110.html#trailer.fields)). However, since trailers are often ignored, it is preferable to send ETag as a header field unless the entity tag is generated while sending the content.
...
The request method's semantics might be further specialized by the semantics of some header fields when present in a request if those additional semantics do not conflict with the method. For example, a client can send conditional request header fields ([Section 13.1](https://www.rfc-editor.org/rfc/rfc9110.html#preconditions)) to make the requested action conditional on the current state of the target resource.

HTTP is designed to be usable as an interface to distributed object systems. The request method invokes an action to be applied to a [target resource](https://www.rfc-editor.org/rfc/rfc9110.html#target.resource) in much the same way that a remote method invocation can be sent to an identified object.
...
Additional methods, outside the scope of this specification, have been specified for use in HTTP. All such methods ought to be registered within the "Hypertext Transfer Protocol (HTTP) Method Registry", as described in [Section 16.1](https://www.rfc-editor.org/rfc/rfc9110.html#method.extensibility).
...
A client can alter the semantics of GET to be a "range request", requesting transfer of only some part(s) of the selected representation, by sending a [Range](https://www.rfc-editor.org/rfc/rfc9110.html#field.range) header field in the request ([Section 14.2](https://www.rfc-editor.org/rfc/rfc9110.html#field.range)).
...
The response to a GET request is cacheable; a cache **MAY** use it to satisfy subsequent GET and HEAD requests unless otherwise indicated by the Cache-Control header field ([Section 5.2](https://www.rfc-editor.org/rfc/rfc9111#section-5.2) of [[CACHING](https://www.rfc-editor.org/rfc/rfc9110.html#CACHING)]).
...
The POST method requests that the [target resource](https://www.rfc-editor.org/rfc/rfc9110.html#target.resource) process the representation enclosed in the request according to the resource's own specific semantics. For example, POST is used for the following functions (among others):
...
3. reject the request with a [415 (Unsupported Media Type)](https://www.rfc-editor.org/rfc/rfc9110.html#status.415) response indicating that the target resource is limited to "text/html", perhaps including a link to a different resource that would be a suitable target for the new representation.
...
- a [202 (Accepted)](https://www.rfc-editor.org/rfc/rfc9110.html#status.202) status code if the action will likely succeed but has not yet been enacted,
- a [204 (No Content)](https://www.rfc-editor.org/rfc/rfc9110.html#status.204) status code if the action has been enacted and no further information is to be supplied, or
- a [200 (OK)](https://www.rfc-editor.org/rfc/rfc9110.html#status.200) status code if the action has been enacted and the response message includes a representation describing the status.
...
A client that generates an OPTIONS request containing content **MUST** send a valid [Content-Type](https://www.rfc-editor.org/rfc/rfc9110.html#field.content-type) header field describing the representation media type. Note that this specification does not define any use for such content.
...
allows the origin server to immediately respond with an error message, such as [401 (Unauthorized)](https://www.rfc-editor.org/rfc/rfc9110.html#status.401) or [405 (Method Not Allowed)](https://www.rfc-editor.org/rfc/rfc9110.html#status.405), before the client starts filling the pipes with an unnecessary data transfer.

Requirements for clients:

- A client **MUST NOT** generate a 100-continue expectation in a request that does not include content.
- A client that will wait for a [100 (Continue)](https://www.rfc-editor.org/rfc/rfc9110.html#status.100) response before sending the request content **MUST** send an [Expect](https://www.rfc-editor.org/rfc/rfc9110.html#field.expect) header field containing a 100-continue expectation.
- A client that sends a 100-continue expectation is not required to wait for any specific length of time; such a client **MAY** proceed to send the content even if it has not yet received a response. Furthermore, since [100 (Continue)](https://www.rfc-editor.org/rfc/rfc9110.html#status.100) responses cannot be sent through an HTTP/1.0 intermediary, such a client **SHOULD NOT** wait for an indefinite period before sending the content.
- A client that receives a [417 (Expectation Failed)](https://www.rfc-editor.org/rfc/rfc9110.html#status.417) status code in response to a request containing a 100-continue expectation **SHOULD** repeat that request without a 100-continue expectation, since the 417 response merely indicates that the response chain does not support expectations (e.g., it passes through an HTTP/1.0 server).

Requirements for servers:

- A server that receives a 100-continue expectation in an HTTP/1.0 request **MUST** ignore that expectation.
- A server **MAY** omit sending a [100 (Continue)](https://www.rfc-editor.org/rfc/rfc9110.html#status.100) response if it has already received some or all of the content for the corresponding request, or if the framing indicates that there is no content.
- A server that sends a [100 (Continue)](https://www.rfc-editor.org/rfc/rfc9110.html#status.100) response **MUST** ultimately send a final status code, once it receives and processes the request content, unless the connection is closed prematurely.
...
- an immediate [100 (Continue)](https://www.rfc-editor.org/rfc/rfc9110.html#status.100) response to encourage the client to send the request content.

The origin server **MUST NOT** wait for the content before sending the [100 (Continue)](https://www.rfc-editor.org/rfc/rfc9110.html#status.100) response.
...
If the proxy believes (from configuration or past interaction) that the next inbound server only supports HTTP/1.0, the proxy **MAY** generate an immediate [100 (Continue)](https://www.rfc-editor.org/rfc/rfc9110.html#status.100) response to encourage the client to begin sending the content.

#### [10.1.2. ](https://www.rfc-editor.org/rfc/rfc9110.html#section-10.1.2)[From](https://www.rfc-editor.org/rfc/rfc9110.html#name-from)



The "From" header field contains an Internet email address for a human user who controls the requesting user agent. The address ought to be machine-usable, as defined by "mailbox" in [Section 3.4](https://www.rfc-editor.org/rfc/rfc5322#section-3.4) of [[RFC5322](https://www.rfc-editor.org/rfc/rfc9110.html#RFC5322)]:
...
As described in [Section 6.5](https://www.rfc-editor.org/rfc/rfc9110.html#trailer.fields), a TE field with a "trailers" member sent in a request indicates that the client will not discard trailer fields.

TE is also used within HTTP/1.1 to advise servers about which transfer codings the client is able to accept in a response. As of publication, only HTTP/1.1 uses transfer codings (see [Section 7](https://www.rfc-editor.org/rfc/rfc9112#section-7) of [[HTTP/1.1](https://www.rfc-editor.org/rfc/rfc9110.html#HTTP11)]).

The TE field value is a list of members, with each member (aside from "trailers") consisting of a transfer coding name token with an optional weight indicating the client's relative preference for that transfer coding ([Section 12.4.2](https://www.rfc-editor.org/rfc/rfc9110.html#quality.values)) and optional parameters for that transfer coding.
...
A sender of TE **MUST** also send a "TE" connection option within the [Connection](https://www.rfc-editor.org/rfc/rfc9110.html#field.connection) header field ([Section 7.6.1](https://www.rfc-editor.org/rfc/rfc9110.html#field.connection)) to inform intermediaries not to forward this field.
...
The response header fields below provide additional information about the response, beyond what is implied by the status code, including information about the server, about the [target resource](https://www.rfc-editor.org/rfc/rfc9110.html#target.resource), or about related resources.
...
The "Allow" header field lists the set of methods advertised as supported by the [target resource](https://www.rfc-editor.org/rfc/rfc9110.html#target.resource). The purpose of this field is strictly to inform the recipient of valid request methods associated with the resource.
...
For example, a GET request generated for the URI reference "http://www.example.org/~tim" might result in a [303 (See Other)](https://www.rfc-editor.org/rfc/rfc9110.html#status.303) response containing the header field:
...
Likewise, a GET request generated for the URI reference "http://www.example.org/index.html#larry" might result in a [301 (Moved Permanently)](https://www.rfc-editor.org/rfc/rfc9110.html#status.301) response containing the header field:
...
There are circumstances in which a fragment identifier in a Location value would not be appropriate. For example, the Location header field in a [201 (Created)](https://www.rfc-editor.org/rfc/rfc9110.html#status.201) response is supposed to provide a URI that is specific to the created resource.
...
A server that receives valid credentials that are not adequate to gain access ought to respond with the [403 (Forbidden)](https://www.rfc-editor.org/rfc/rfc9110.html#status.403) status code ([Section 15.5.4](https://www.rfc-editor.org/rfc/rfc9110.html#status.403)).
...
Note that various custom mechanisms for user authentication use the Set-Cookie and Cookie header fields, defined in [[COOKIE](https://www.rfc-editor.org/rfc/rfc9110.html#COOKIE)], for passing tokens related to authentication.
...
A proxy forwarding a response **MUST NOT** modify any [WWW-Authenticate](https://www.rfc-editor.org/rfc/rfc9110.html#field.www-authenticate) header fields in that response.
...
Note that the parsing considerations for [WWW-Authenticate](https://www.rfc-editor.org/rfc/rfc9110.html#field.www-authenticate) apply to this header field as well; see [Section 11.6.1](https://www.rfc-editor.org/rfc/rfc9110.html#field.www-authenticate) for details.
...
Proxy-Authentication-Info can be sent as a trailer field ([Section 6.5](https://www.rfc-editor.org/rfc/rfc9110.html#trailer.fields)) when the authentication scheme explicitly allows this.
...
A [Vary](https://www.rfc-editor.org/rfc/rfc9110.html#field.vary) header field ([Section 12.5.5](https://www.rfc-editor.org/rfc/rfc9110.html#field.vary)) is often sent in a response subject to proactive negotiation to indicate what parts of the request information were used in the selection algorithm.
...
When content negotiation preferences are sent in a server's response, the listed preferences are called "request content negotiation" because they intend to influence selection of an appropriate content for subsequent requests to that resource. For example, the [Accept](https://www.rfc-editor.org/rfc/rfc9110.html#field.accept) ([Section 12.5.1](https://www.rfc-editor.org/rfc/rfc9110.html#field.accept)) and [Accept-Encoding](https://www.rfc-editor.org/rfc/rfc9110.html#field.accept-encoding) ([Section 12.5.3](https://www.rfc-editor.org/rfc/rfc9110.html#field.accept-encoding)) header fields can be sent in a response to indicate preferred media types and content codings for subsequent requests to that resource.

Similarly, [Section 3.1](https://www.rfc-editor.org/rfc/rfc5789#section-3.1) of [[RFC5789](https://www.rfc-editor.org/rfc/rfc9110.html#RFC5789)] defines the "Accept-Patch" response header field, which allows discovery of which content types are accepted in PATCH requests.
...
**Note:** A user agent sending these header fields makes it easier for a server to identify an individual by virtue of the user agent's request characteristics ([Section 17.13](https://www.rfc-editor.org/rfc/rfc9110.html#fingerprinting)).
...
Each media-range might be followed by optional applicable media type parameters (e.g., [charset](https://www.rfc-editor.org/rfc/rfc9110.html#charset)), followed by an optional "q" parameter for indicating a relative weight ([Section 12.4.2](https://www.rfc-editor.org/rfc/rfc9110.html#quality.values)).
...
The "Accept-Encoding" header field can be used to indicate preferences regarding the use of content codings ([Section 8.4.1](https://www.rfc-editor.org/rfc/rfc9110.html#content.codings)).
...
3. If the representation's content coding is one of the content codings listed in the Accept-Encoding field value, then it is acceptable unless it is accompanied by a qvalue of 0. (As defined in [Section 12.4.2](https://www.rfc-editor.org/rfc/rfc9110.html#quality.values), a qvalue of 0 means "not acceptable".)
...
The "Accept-Language" header field can be used by user agents to indicate the set of natural languages that are preferred in the response. Language tags are defined in [Section 8.5.1](https://www.rfc-editor.org/rfc/rfc9110.html#language.tags).
...
Each language-range can be given an associated quality value representing an estimate of the user's preference for the languages specified by that range, as defined in [Section 12.4.2](https://www.rfc-editor.org/rfc/rfc9110.html#quality.values). For example,
...
It might be contrary to the privacy expectations of the user to send an Accept-Language header field with the complete linguistic preferences of the user in every request ([Section 17.13](https://www.rfc-editor.org/rfc/rfc9110.html#fingerprinting)).
...
A recipient **MUST** use the weak comparison function when comparing entity tags for If-None-Match ([Section 8.8.3.2](https://www.rfc-editor.org/rfc/rfc9110.html#entity.tag.comparison)), since weak entity tags can be used for cache validation even if there have been changes to the representation data.
...
Requirements on cache handling of a received If-None-Match header field are defined in [Section 4.3.2](https://www.rfc-editor.org/rfc/rfc9111#section-4.3.2) of [[CACHING](https://www.rfc-editor.org/rfc/rfc9110.html#CACHING)].
...
Requirements on cache handling of a received If-Modified-Since header field are defined in [Section 4.3.2](https://www.rfc-editor.org/rfc/rfc9111#section-4.3.2) of [[CACHING](https://www.rfc-editor.org/rfc/rfc9110.html#CACHING)].
...
A valid [entity-tag](https://www.rfc-editor.org/rfc/rfc9110.html#field.etag) can be distinguished from a valid [HTTP-date](https://www.rfc-editor.org/rfc/rfc9110.html#http.date) by examining the first three characters for a DQUOTE.

A client **MUST NOT** generate an If-Range header field in a request that does not contain a [Range](https://www.rfc-editor.org/rfc/rfc9110.html#field.range) header field. A server **MUST** ignore an If-Range header field received in a request that does not contain a [Range](https://www.rfc-editor.org/rfc/rfc9110.html#field.range) header field. An origin server **MUST** ignore an If-Range header field received in a request for a target resource that does not support Range requests.
...
A server that receives an If-Range header field on a Range request **MUST** evaluate the condition per [Section 13.2](https://www.rfc-editor.org/rfc/rfc9110.html#evaluation) prior to performing the method.

To evaluate a received If-Range header field containing an [HTTP-date](https://www.rfc-editor.org/rfc/rfc9110.html#http.date):

1. If the [HTTP-date](https://www.rfc-editor.org/rfc/rfc9110.html#http.date) validator provided is not a strong validator in the sense defined by [Section 8.8.2.2](https://www.rfc-editor.org/rfc/rfc9110.html#lastmod.comparison), the condition is false.
2. If the [HTTP-date](https://www.rfc-editor.org/rfc/rfc9110.html#http.date) validator provided exactly matches the [Last-Modified](https://www.rfc-editor.org/rfc/rfc9110.html#field.last-modified) field value for the selected representation, the condition is true.
3. Otherwise, the condition is false.

To evaluate a received If-Range header field containing an [entity-tag](https://www.rfc-editor.org/rfc/rfc9110.html#field.etag):

1. If the [entity-tag](https://www.rfc-editor.org/rfc/rfc9110.html#field.etag) validator provided exactly matches the [ETag](https://www.rfc-editor.org/rfc/rfc9110.html#field.etag) field value for the selected representation using the strong comparison function ([Section 8.8.3.2](https://www.rfc-editor.org/rfc/rfc9110.html#entity.tag.comparison)), the condition is true.
2. Otherwise, the condition is false.

A recipient of an If-Range header field **MUST** ignore the [Range](https://www.rfc-editor.org/rfc/rfc9110.html#field.range) header field if the If-Range condition evaluates to false. Otherwise, the recipient **SHOULD** process the [Range](https://www.rfc-editor.org/rfc/rfc9110.html#field.range) header field as requested.

Note that the If-Range comparison is by exact match, including when the validator is an [HTTP-date](https://www.rfc-editor.org/rfc/rfc9110.html#http.date), and so it differs from the "earlier than or equal to" comparison used when evaluating an [If-Unmodified-Since](https://www.rfc-editor.org/rfc/rfc9110.html#field.if-unmodified-since) conditional.
...
1. When recipient is the origin server and [If-Match](https://www.rfc-editor.org/rfc/rfc9110.html#field.if-match) is present, evaluate the [If-Match](https://www.rfc-editor.org/rfc/rfc9110.html#field.if-match) precondition:
   - if true, continue to step [3](https://www.rfc-editor.org/rfc/rfc9110.html#precedence3)
   - if false, respond [412 (Precondition Failed)](https://www.rfc-editor.org/rfc/rfc9110.html#status.412) unless it can be determined that the state-changing request has already succeeded (see [Section 13.1.1](https://www.rfc-editor.org/rfc/rfc9110.html#field.if-match))
2. When recipient is the origin server, [If-Match](https://www.rfc-editor.org/rfc/rfc9110.html#field.if-match) is not present, and [If-Unmodified-Since](https://www.rfc-editor.org/rfc/rfc9110.html#field.if-unmodified-since) is present, evaluate the [If-Unmodified-Since](https://www.rfc-editor.org/rfc/rfc9110.html#field.if-unmodified-since) precondition:
   - if true, continue to step [3](https://www.rfc-editor.org/rfc/rfc9110.html#precedence3)
   - if false, respond [412 (Precondition Failed)](https://www.rfc-editor.org/rfc/rfc9110.html#status.412) unless it can be determined that the state-changing request has already succeeded (see [Section 13.1.4](https://www.rfc-editor.org/rfc/rfc9110.html#field.if-unmodified-since))
3. When [If-None-Match](https://www.rfc-editor.org/rfc/rfc9110.html#field.if-none-match) is present, evaluate the [If-None-Match](https://www.rfc-editor.org/rfc/rfc9110.html#field.if-none-match) precondition:
   - if true, continue to step [5](https://www.rfc-editor.org/rfc/rfc9110.html#precedence5)
   - if false for GET/HEAD, respond [304 (Not Modified)](https://www.rfc-editor.org/rfc/rfc9110.html#status.304)
   - if false for other methods, respond [412 (Precondition Failed)](https://www.rfc-editor.org/rfc/rfc9110.html#status.412)
4. When the method is GET or HEAD, [If-None-Match](https://www.rfc-editor.org/rfc/rfc9110.html#field.if-none-match) is not present, and [If-Modified-Since](https://www.rfc-editor.org/rfc/rfc9110.html#field.if-modified-since) is present, evaluate the [If-Modified-Since](https://www.rfc-editor.org/rfc/rfc9110.html#field.if-modified-since) precondition:
   - if true, continue to step [5](https://www.rfc-editor.org/rfc/rfc9110.html#precedence5)
   - if false, respond [304 (Not Modified)](https://www.rfc-editor.org/rfc/rfc9110.html#status.304)
5. When the method is GET and both [Range](https://www.rfc-editor.org/rfc/rfc9110.html#field.range) and [If-Range](https://www.rfc-editor.org/rfc/rfc9110.html#field.if-range) are present, evaluate the [If-Range](https://www.rfc-editor.org/rfc/rfc9110.html#field.if-range) precondition:
   - if true and the [Range](https://www.rfc-editor.org/rfc/rfc9110.html#field.range) is applicable to the [selected representation](https://www.rfc-editor.org/rfc/rfc9110.html#selected.representation), respond [206 (Partial Content)](https://www.rfc-editor.org/rfc/rfc9110.html#status.206)
   - otherwise, ignore the [Range](https://www.rfc-editor.org/rfc/rfc9110.html#field.range) header field and respond [200 (OK)](https://www.rfc-editor.org/rfc/rfc9110.html#status.200)
6. Otherwise,
...
All range unit names are case-insensitive and ought to be registered within the "HTTP Range Unit Registry", as defined in [Section 16.5.1](https://www.rfc-editor.org/rfc/rfc9110.html#range.unit.registry).

Range units are intended to be extensible, as described in [Section 16.5](https://www.rfc-editor.org/rfc/rfc9110.html#range.unit.extensibility).
...
A [suffix-range](https://www.rfc-editor.org/rfc/rfc9110.html#rule.suffix-range) is a range expressed as a suffix of the representation data with the provided non-negative integer maximum length (in range units). In other words, the last N units of the representation data.
...
To provide for extensibility, the [other-range](https://www.rfc-editor.org/rfc/rfc9110.html#rule.other-range) rule is a mostly unconstrained grammar that allows application-specific or future range units to define additional range specifiers.
...
For a [GET](https://www.rfc-editor.org/rfc/rfc9110.html#GET) request, a valid bytes [range-spec](https://www.rfc-editor.org/rfc/rfc9110.html#rule.ranges-specifier) is [satisfiable](https://www.rfc-editor.org/rfc/rfc9110.html#satisfiable) if it is either:

- an [int-range](https://www.rfc-editor.org/rfc/rfc9110.html#rule.int-range) with a [first-pos](https://www.rfc-editor.org/rfc/rfc9110.html#rule.int-range) that is less than the current length of the selected representation or
- a [suffix-range](https://www.rfc-editor.org/rfc/rfc9110.html#rule.suffix-range) with a non-zero [suffix-length](https://www.rfc-editor.org/rfc/rfc9110.html#rule.suffix-range).
...
A server **MUST** ignore a Range header field received with a request method that is unrecognized or for which range handling is not defined. For this specification, [GET](https://www.rfc-editor.org/rfc/rfc9110.html#GET) is the only method for which range handling is defined.
...
For example, a server that supports [byte-range requests](https://www.rfc-editor.org/rfc/rfc9110.html#byte.ranges) ([Section 14.1.2](https://www.rfc-editor.org/rfc/rfc9110.html#byte.ranges)) can send the field
...
to indicate that it supports byte range requests for that target resource, thereby encouraging its use by the client for future partial requests on the same request path. Range units are defined in [Section 14.1](https://www.rfc-editor.org/rfc/rfc9110.html#range.units).
...
A server generating a [416 (Range Not Satisfiable)](https://www.rfc-editor.org/rfc/rfc9110.html#status.416) response to a byte-range request **SHOULD** send a Content-Range header field with an [unsatisfied-range](https://www.rfc-editor.org/rfc/rfc9110.html#field.content-range) value, as in the following example:
...
Some origin servers support [PUT](https://www.rfc-editor.org/rfc/rfc9110.html#PUT) of a partial representation when the user agent sends a [Content-Range](https://www.rfc-editor.org/rfc/rfc9110.html#field.content-range) header field ([Section 14.4](https://www.rfc-editor.org/rfc/rfc9110.html#field.content-range)) in the request, though such support is inconsistent and depends on private agreements with user agents. In general, it requests that the state of the [target resource](https://www.rfc-editor.org/rfc/rfc9110.html#target.resource) be partly replaced with the enclosed content at an offset and length indicated by the Content-Range value, where the offset is relative to the current selected representation.

An origin server **SHOULD** respond with a [400 (Bad Request)](https://www.rfc-editor.org/rfc/rfc9110.html#status.400) status code if it receives [Content-Range](https://www.rfc-editor.org/rfc/rfc9110.html#field.content-range) on a PUT for a target resource that does not support partial PUT requests.
...
- Security considerations:
  ...
  see [Section 17](https://www.rfc-editor.org/rfc/rfc9110.html#security.considerations)
...
- Published specification:
  ...
  RFC 9110 (see [Section 14.6](https://www.rfc-editor.org/rfc/rfc9110.html#multipart.byteranges))
...
- Additional information:
  ...
  Deprecated alias names for this type:N/AMagic number(s):N/AFile extension(s):N/AMacintosh file type code(s):N/A
...
- [1xx (Informational)](https://www.rfc-editor.org/rfc/rfc9110.html#status.1xx): The request was received, continuing process
- [2xx (Successful)](https://www.rfc-editor.org/rfc/rfc9110.html#status.2xx): The request was successfully received, understood, and accepted
- [3xx (Redirection)](https://www.rfc-editor.org/rfc/rfc9110.html#status.3xx): Further action needs to be taken in order to complete the request
- [4xx (Client Error)](https://www.rfc-editor.org/rfc/rfc9110.html#status.4xx): The request contains bad syntax or cannot be fulfilled
- [5xx (Server Error)](https://www.rfc-editor.org/rfc/rfc9110.html#status.5xx): The server failed to fulfill an apparently valid request
...
A single request can have multiple associated responses: zero or more "interim" (non-final) responses with status codes in the "informational" ([1xx](https://www.rfc-editor.org/rfc/rfc9110.html#status.1xx)) range, followed by exactly one "final" response with a status code in one of the other ranges.
...
If the request did not contain an [Expect](https://www.rfc-editor.org/rfc/rfc9110.html#field.expect) header field containing the [100-continue](https://www.rfc-editor.org/rfc/rfc9110.html#field.expect) expectation, the client can simply discard this interim response.
...
A 203 response is heuristically cacheable; i.e., unless otherwise indicated by the method definition or explicit cache controls (see [Section 4.2.2](https://www.rfc-editor.org/rfc/rfc9111#section-4.2.2) of [[CACHING](https://www.rfc-editor.org/rfc/rfc9110.html#CACHING)]).
...
For example, if a 204 status code is received in response to a PUT request and the response contains an [ETag](https://www.rfc-editor.org/rfc/rfc9110.html#field.etag) field, then the PUT was successful and the ETag field value contains the entity tag for the new representation of that target resource.
...
A 204 response is heuristically cacheable; i.e., unless otherwise indicated by the method definition or explicit cache controls (see [Section 4.2.2](https://www.rfc-editor.org/rfc/rfc9111#section-4.2.2) of [[CACHING](https://www.rfc-editor.org/rfc/rfc9110.html#CACHING)]).
...
The 206 (Partial Content) status code indicates that the server is successfully fulfilling a range request for the target resource by transferring one or more parts of the [selected representation](https://www.rfc-editor.org/rfc/rfc9110.html#selected.representation).
...
A client **MUST** inspect a 206 response's [Content-Type](https://www.rfc-editor.org/rfc/rfc9110.html#field.content-type) and [Content-Range](https://www.rfc-editor.org/rfc/rfc9110.html#field.content-range) field(s) to determine what parts are enclosed and whether additional requests are needed.
...
A 206 response is heuristically cacheable; i.e., unless otherwise indicated by explicit cache controls (see [Section 4.2.2](https://www.rfc-editor.org/rfc/rfc9111#section-4.2.2) of [[CACHING](https://www.rfc-editor.org/rfc/rfc9110.html#CACHING)]).
...
If a single part is being transferred, the server generating the 206 response **MUST** generate a [Content-Range](https://www.rfc-editor.org/rfc/rfc9110.html#field.content-range) header field, describing what range of the selected representation is enclosed, and a content consisting of the range. For example:
...
If the most recent response is an incomplete [200 (OK)](https://www.rfc-editor.org/rfc/rfc9110.html#status.200) response, then the header fields of that response are used for any combined response and replace those of the matching stored responses.
...
2. Redirection that offers a choice among matching resources capable of representing this resource, as in the [300 (Multiple Choices)](https://www.rfc-editor.org/rfc/rfc9110.html#status.300) status code.
3. Redirection to a different resource, identified by the [Location](https://www.rfc-editor.org/rfc/rfc9110.html#field.location) header field, that can represent an indirect response to the request, as in the [303 (See Other)](https://www.rfc-editor.org/rfc/rfc9110.html#status.303) status code.
4. Redirection to a previously stored result, as in the [304 (Not Modified)](https://www.rfc-editor.org/rfc/rfc9110.html#status.304) status code.
...
1. Replace the target URI with the URI referenced by the redirection response's [Location](https://www.rfc-editor.org/rfc/rfc9110.html#field.location) header field value after resolving it relative to the original request's target URI.
2. Remove header fields that were automatically generated by the implementation, replacing them with updated values as appropriate to the new request. This includes:
   1. Connection-specific header fields (see [Section 7.6.1](https://www.rfc-editor.org/rfc/rfc9110.html#field.connection)),
   2. Header fields specific to the client's proxy configuration, including (but not limited to) [Proxy-Authorization](https://www.rfc-editor.org/rfc/rfc9110.html#field.proxy-authorization),
   3. Origin-specific header fields (if any), including (but not limited to) [Host](https://www.rfc-editor.org/rfc/rfc9110.html#field.host),
   4. Validating header fields that were added by the implementation's cache (e.g., [If-None-Match](https://www.rfc-editor.org/rfc/rfc9110.html#field.if-none-match), [If-Modified-Since](https://www.rfc-editor.org/rfc/rfc9110.html#field.if-modified-since)), and
   5. Resource-specific header fields, including (but not limited to) [Referer](https://www.rfc-editor.org/rfc/rfc9110.html#field.referer), Origin, [Authorization](https://www.rfc-editor.org/rfc/rfc9110.html#field.authorization), and Cookie.
...
If the server has a preferred choice, the server **SHOULD** generate a [Location](https://www.rfc-editor.org/rfc/rfc9110.html#field.location) header field containing a preferred choice's URI reference. The user agent **MAY** use the Location field value for automatic redirection.
...
A 300 response is heuristically cacheable; i.e., unless otherwise indicated by the method definition or explicit cache controls (see [Section 4.2.2](https://www.rfc-editor.org/rfc/rfc9111#section-4.2.2) of [[CACHING](https://www.rfc-editor.org/rfc/rfc9110.html#CACHING)]).
...
**Note:** For historical reasons, a user agent **MAY** change the request method from POST to GET for the subsequent request. If this behavior is undesired, the [308 (Permanent Redirect)](https://www.rfc-editor.org/rfc/rfc9110.html#status.308) status code can be used instead.

A 301 response is heuristically cacheable; i.e., unless otherwise indicated by the method definition or explicit cache controls (see [Section 4.2.2](https://www.rfc-editor.org/rfc/rfc9111#section-4.2.2) of [[CACHING](https://www.rfc-editor.org/rfc/rfc9110.html#CACHING)]).
...
**Note:** For historical reasons, a user agent **MAY** change the request method from POST to GET for the subsequent request. If this behavior is undesired, the [307 (Temporary Redirect)](https://www.rfc-editor.org/rfc/rfc9110.html#status.307) status code can be used instead.
...
Except for responses to a HEAD request, the representation of a 303 response ought to contain a short hypertext note with a hyperlink to the same URI reference provided in the [Location](https://www.rfc-editor.org/rfc/rfc9110.html#field.location) header field.
...
- Cache-Control and Expires (see [[CACHING](https://www.rfc-editor.org/rfc/rfc9110.html#CACHING)])
...
The 305 (Use Proxy) status code was defined in a previous version of this specification and is now deprecated ([Appendix B](https://www.rfc-editor.org/rfc/rfc7231#appendix-B) of [[RFC7231](https://www.rfc-editor.org/rfc/rfc9110.html#RFC7231)]).
...
A 308 response is heuristically cacheable; i.e., unless otherwise indicated by the method definition or explicit cache controls (see [Section 4.2.2](https://www.rfc-editor.org/rfc/rfc9111#section-4.2.2) of [[CACHING](https://www.rfc-editor.org/rfc/rfc9110.html#CACHING)]).

**Note:** This status code is much younger (June 2014) than its sibling codes and thus might not be recognized everywhere. See [Section 4](https://www.rfc-editor.org/rfc/rfc7538#section-4) of [[RFC7538](https://www.rfc-editor.org/rfc/rfc9110.html#RFC7538)] for deployment considerations.
...
A 404 response is heuristically cacheable; i.e., unless otherwise indicated by the method definition or explicit cache controls (see [Section 4.2.2](https://www.rfc-editor.org/rfc/rfc9111#section-4.2.2) of [[CACHING](https://www.rfc-editor.org/rfc/rfc9110.html#CACHING)]).
...
A 405 response is heuristically cacheable; i.e., unless otherwise indicated by the method definition or explicit cache controls (see [Section 4.2.2](https://www.rfc-editor.org/rfc/rfc9111#section-4.2.2) of [[CACHING](https://www.rfc-editor.org/rfc/rfc9110.html#CACHING)]).
...
A 410 response is heuristically cacheable; i.e., unless otherwise indicated by the method definition or explicit cache controls (see [Section 4.2.2](https://www.rfc-editor.org/rfc/rfc9111#section-4.2.2) of [[CACHING](https://www.rfc-editor.org/rfc/rfc9110.html#CACHING)]).
...
If the condition is temporary, the server **SHOULD** generate a [Retry-After](https://www.rfc-editor.org/rfc/rfc9110.html#field.retry-after) header field to indicate that it is temporary and after what time the client **MAY** try again.
...
A 414 response is heuristically cacheable; i.e., unless otherwise indicated by the method definition or explicit cache controls (see [Section 4.2.2](https://www.rfc-editor.org/rfc/rfc9111#section-4.2.2) of [[CACHING](https://www.rfc-editor.org/rfc/rfc9110.html#CACHING)]).
...
The 415 (Unsupported Media Type) status code indicates that the origin server is refusing to service the request because the content is in a format not supported by this method on the [target resource](https://www.rfc-editor.org/rfc/rfc9110.html#target.resource).

The format problem might be due to the request's indicated [Content-Type](https://www.rfc-editor.org/rfc/rfc9110.html#field.content-type) or [Content-Encoding](https://www.rfc-editor.org/rfc/rfc9110.html#field.content-encoding), or as a result of inspecting the data directly.
...
On the other hand, if the cause was an unsupported media type, the [Accept](https://www.rfc-editor.org/rfc/rfc9110.html#field.accept) response header field ([Section 12.5.1](https://www.rfc-editor.org/rfc/rfc9110.html#field.accept)) can be used to indicate which media types would have been accepted in the request.
...
Each range unit defines what is required for its own range sets to be satisfiable. For example, [Section 14.1.2](https://www.rfc-editor.org/rfc/rfc9110.html#byte.ranges) defines what makes a bytes range set satisfiable.
...
The 417 (Expectation Failed) status code indicates that the expectation given in the request's [Expect](https://www.rfc-editor.org/rfc/rfc9110.html#field.expect) header field ([Section 10.1.1](https://www.rfc-editor.org/rfc/rfc9110.html#field.expect)) could not be met by at least one of the inbound servers.
...
A 501 response is heuristically cacheable; i.e., unless otherwise indicated by the method definition or explicit cache controls (see [Section 4.2.2](https://www.rfc-editor.org/rfc/rfc9111#section-4.2.2) of [[CACHING](https://www.rfc-editor.org/rfc/rfc9110.html#CACHING)]).
...
The 505 (HTTP Version Not Supported) status code indicates that the server does not support, or refuses to support, the major version of HTTP that was used in the request message. The server is indicating that it is unable or unwilling to complete the request using the same major version as the client, as described in [Section 2.5](https://www.rfc-editor.org/rfc/rfc9110.html#protocol.version), other than with this error message. The server **SHOULD** generate a representation for the 505 response that describes why that version is not supported and what other protocols are supported by that server.
...
The "Hypertext Transfer Protocol (HTTP) Method Registry", maintained by IANA at <https://www.iana.org/assignments/http-methods>, registers [method](https://www.rfc-editor.org/rfc/rfc9110.html#method.overview) names.

HTTP method registrations **MUST** include the following fields:

- Method Name (see [Section 9](https://www.rfc-editor.org/rfc/rfc9110.html#methods))
- Safe ("yes" or "no", see [Section 9.2.1](https://www.rfc-editor.org/rfc/rfc9110.html#safe.methods))
- Idempotent ("yes" or "no", see [Section 9.2.2](https://www.rfc-editor.org/rfc/rfc9110.html#idempotent.methods))
- Pointer to specification text

Values to be added to this namespace require IETF Review (see [[RFC8126](https://www.rfc-editor.org/rfc/rfc9110.html#RFC8126)], [Section 4.8](https://www.rfc-editor.org/rfc/rfc8126#section-4.8)).
...
Values to be added to the HTTP status code namespace require IETF Review (see [[RFC8126](https://www.rfc-editor.org/rfc/rfc9110.html#RFC8126)], [Section 4.8](https://www.rfc-editor.org/rfc/rfc8126#section-4.8)).
...
New status codes are required to fall under one of the categories defined in [Section 15](https://www.rfc-editor.org/rfc/rfc9110.html#status.codes). To allow existing parsers to process the response message, new status codes cannot disallow content, although they can mandate a zero-length content.
...
Finally, the definition of a new status code ought to indicate whether the content has any implied association with an identified resource ([Section 6.4.2](https://www.rfc-editor.org/rfc/rfc9110.html#identifying.content)).
...
Any party can request registration of an HTTP field. See [Section 16.3.2](https://www.rfc-editor.org/rfc/rfc9110.html#considerations.for.new.fields) for considerations to take into account when creating a new HTTP field.
...
Field names are registered on the advice of a designated expert (appointed by the IESG or their delegate). Fields with the status 'permanent' are Specification Required ([[RFC8126](https://www.rfc-editor.org/rfc/rfc9110.html#RFC8126)], [Section 4.6](https://www.rfc-editor.org/rfc/rfc8126#section-4.6)).
...
- Field name:
  ...
  The requested field name. It **MUST** conform to the field-name syntax defined in [Section 5.1](https://www.rfc-editor.org/rfc/rfc9110.html#fields.names), and it **SHOULD** be restricted to just letters, digits, and hyphen ('-') characters, with the first character being a letter.
...
- If the field is allowable in trailers; by default, it will not be (see [Section 6.5.1](https://www.rfc-editor.org/rfc/rfc9110.html#trailers.limitations)).
- Whether it is appropriate or even required to list the field name in the [Connection](https://www.rfc-editor.org/rfc/rfc9110.html#field.connection) header field (i.e., if the field is to be hop-by-hop; see [Section 7.6.1](https://www.rfc-editor.org/rfc/rfc9110.html#field.connection)).
...
- If it is appropriate to list the field name in a [Vary](https://www.rfc-editor.org/rfc/rfc9110.html#field.vary) response header field (e.g., when the request header field is used by an origin server's content selection algorithm; see [Section 12.5.5](https://www.rfc-editor.org/rfc/rfc9110.html#field.vary)).
- If the field is intended to be stored when received in a PUT request (see [Section 9.3.4](https://www.rfc-editor.org/rfc/rfc9110.html#PUT)).
- If the field ought to be removed when automatically redirecting a request due to security concerns (see [Section 15.4](https://www.rfc-editor.org/rfc/rfc9110.html#status.3xx)).
...
Field names ought not be prefixed with "X-"; see [[BCP178](https://www.rfc-editor.org/rfc/rfc9110.html#BCP178)] for further information.

Other prefixes are sometimes used in HTTP field names; for example, "Accept-" is used in many content negotiation headers, and "Content-" is used as explained in [Section 6.4](https://www.rfc-editor.org/rfc/rfc9110.html#content). These prefixes are only an aid to recognizing the purpose of a field and do not trigger automatic processing.
...
Authors are encouraged (but not required) to use either the ABNF rules in this specification or those in [[RFC8941](https://www.rfc-editor.org/rfc/rfc9110.html#RFC8941)] to define the syntax of new field values.
...
- The authentication parameter "realm" is reserved for defining protection spaces as described in [Section 11.5](https://www.rfc-editor.org/rfc/rfc9110.html#protection.space). New schemes **MUST NOT** use it in a way incompatible with that definition.
...
The "HTTP Content Coding Registry", maintained by IANA at <https://www.iana.org/assignments/http-parameters/>, registers [content-coding](https://www.rfc-editor.org/rfc/rfc9110.html#content.codings) names.
...
The "Hypertext Transfer Protocol (HTTP) Upgrade Token Registry" defines the namespace for protocol-name tokens used to identify protocols in the [Upgrade](https://www.rfc-editor.org/rfc/rfc9110.html#field.upgrade) header field. The registry is maintained at <https://www.iana.org/assignments/http-upgrade-tokens>.
...
Registrations happen on a "First Come First Served" basis (see [Section 4.4](https://www.rfc-editor.org/rfc/rfc8126#section-4.4) of [[RFC8126](https://www.rfc-editor.org/rfc/rfc9110.html#RFC8126)]) and are subject to the following rules:
...
Intermediaries that contain a shared cache are especially vulnerable to cache poisoning attacks, as described in [Section 7](https://www.rfc-editor.org/rfc/rfc9111#section-7) of [[CACHING](https://www.rfc-editor.org/rfc/rfc9110.html#CACHING)].
...
To promote interoperability, specific recommendations are made for minimum size limits on fields ([Section 5.4](https://www.rfc-editor.org/rfc/rfc9110.html#fields.limits)). These are minimum recommendations, chosen to be supportable even by implementations with limited resources; it is expected that most implementations will choose substantially higher limits.
...
IANA has updated the "Hypertext Transfer Protocol (HTTP) Method Registry" at <https://www.iana.org/assignments/http-methods> with the registration procedure of [Section 16.1.1](https://www.rfc-editor.org/rfc/rfc9110.html#method.registry) and the method names summarized in the following table.
...
IANA has updated the "Hypertext Transfer Protocol (HTTP) Status Code Registry" at <https://www.iana.org/assignments/http-status-codes> with the registration procedure of [Section 16.2.1](https://www.rfc-editor.org/rfc/rfc9110.html#status.code.registry) and the status code values summarized in the following table.
...
IANA has created a new registry titled "Hypertext Transfer Protocol (HTTP) Field Name Registry" as outlined in [Section 16.3.1](https://www.rfc-editor.org/rfc/rfc9110.html#fields.registry).
...
The field name "*" is reserved because using that name as an HTTP header field might conflict with its special semantics in the [Vary](https://www.rfc-editor.org/rfc/rfc9110.html#field.vary) header field ([Section 12.5.5](https://www.rfc-editor.org/rfc/rfc9110.html#field.vary)).
...
IANA has updated the "HTTP Content Coding Registry" at <https://www.iana.org/assignments/http-parameters/> with the registration procedure of [Section 16.6.1](https://www.rfc-editor.org/rfc/rfc9110.html#content.coding.registry) and the content coding names summarized in the table below.
...
IANA has updated the "HTTP Range Unit Registry" at <https://www.iana.org/assignments/http-parameters/> with the registration procedure of [Section 16.5.1](https://www.rfc-editor.org/rfc/rfc9110.html#range.unit.registry) and the range unit names summarized in the table below.
...
IANA has updated the "Media Types" registry at <https://www.iana.org/assignments/media-types> with the registration information in [Section 14.6](https://www.rfc-editor.org/rfc/rfc9110.html#multipart.byteranges) for the media type "multipart/byteranges".

IANA has updated the registry note about "q" parameters with a link to [Section 12.5.1](https://www.rfc-editor.org/rfc/rfc9110.html#field.accept) of this document.
...
- [BCP13]
  ...
  Freed, N. and J. Klensin, "Multipurpose Internet Mail Extensions (MIME) Part Four: Registration Procedures", BCP 13, RFC 4289, December 2005.Freed, N., Klensin, J., and T. Hansen, "Media Type Specifications and Registration Procedures", BCP 13, RFC 6838, January 2013.<https://www.rfc-editor.org/info/bcp13>

  

- [BCP178]

  Saint-Andre, P., Crocker, D., and M. Nottingham, "Deprecating the "X-" Prefix and Similar Constructs in Application Protocols", BCP 178, RFC 6648, June 2012.<https://www.rfc-editor.org/info/bcp178>

  

- [BCP35]

  Thaler, D., Ed., Hansen, T., and T. Hardie, "Guidelines and Registration Procedures for URI Schemes", BCP 35, RFC 7595, June 2015.<https://www.rfc-editor.org/info/bcp35>

  

- [BREACH]

  Gluck, Y., Harris, N., and A. Prado, "BREACH: Reviving the CRIME Attack", July 2013, <[http://breachattack.com/resources/BREACH%20-%20SSL,%20gone%20in%2030%20seconds.pdf](http://breachattack.com/resources/BREACH - SSL, gone in 30 seconds.pdf)>.
...
In the collected ABNF below, list rules are expanded per [Section 5.6.1](https://www.rfc-editor.org/rfc/rfc9110.html#abnf.extension).
...
The requirement on semantic conformance has been replaced with permission to ignore or work around implementation-specific failures. ([Section 2.2](https://www.rfc-editor.org/rfc/rfc9110.html#requirements.notation))
...
Explicit requirements have been added to check the target URI scheme's semantics and reject requests that don't meet any associated requirements. ([Section 7.4](https://www.rfc-editor.org/rfc/rfc9110.html#routing.reject))

Parameters in media type, media range, and expectation can be empty via one or more trailing semicolons. ([Section 5.6.6](https://www.rfc-editor.org/rfc/rfc9110.html#parameter))

"Field value" now refers to the value after multiple field lines are combined with commas -- by far the most common use. To refer to a single header line's value, use "field line value". ([Section 6.3](https://www.rfc-editor.org/rfc/rfc9110.html#header.fields))
...
The priority of the absolute form of the request URI over the Host header field by origin servers has been made explicit to align with proxy handling. ([Section 7.2](https://www.rfc-editor.org/rfc/rfc9110.html#field.host))
...
Minimum URI lengths to be supported by implementations are now recommended. ([Section 4.1](https://www.rfc-editor.org/rfc/rfc9110.html#uri.references))

The following have been clarified: CR and NUL in field values are to be rejected or mapped to SP, and leading and trailing whitespace needs to be stripped from field values before they are consumed. ([Section 5.5](https://www.rfc-editor.org/rfc/rfc9110.html#fields.values))
...
The terms "payload" and "payload body" have been replaced with "content", to better align with its usage elsewhere (e.g., in field names) and to avoid confusion with frame payloads in HTTP/2 and HTTP/3. ([Section 6.4](https://www.rfc-editor.org/rfc/rfc9110.html#content))

The term "effective request URI" has been replaced with "target URI". ([Section 7.1](https://www.rfc-editor.org/rfc/rfc9110.html#target.resource))

Restrictions on client retries have been loosened to reflect implementation behavior. ([Section 9.2.2](https://www.rfc-editor.org/rfc/rfc9110.html#idempotent.methods))

The fact that request bodies on GET, HEAD, and DELETE are not interoperable has been clarified. (Sections [9.3.1](https://www.rfc-editor.org/rfc/rfc9110.html#GET), [9.3.2](https://www.rfc-editor.org/rfc/rfc9110.html#HEAD), and [9.3.5](https://www.rfc-editor.org/rfc/rfc9110.html#DELETE))

The use of the [Content-Range](https://www.rfc-editor.org/rfc/rfc9110.html#field.content-range) header field ([Section 14.4](https://www.rfc-editor.org/rfc/rfc9110.html#field.content-range)) as a request modifier on PUT is allowed. ([Section 9.3.4](https://www.rfc-editor.org/rfc/rfc9110.html#PUT))

A superfluous requirement about setting [Content-Length](https://www.rfc-editor.org/rfc/rfc9110.html#field.content-length) has been removed from the description of the OPTIONS method. ([Section 9.3.7](https://www.rfc-editor.org/rfc/rfc9110.html#OPTIONS))

The normative requirement to use the "message/http" media type in TRACE responses has been removed. ([Section 9.3.8](https://www.rfc-editor.org/rfc/rfc9110.html#TRACE))

List-based grammar for [Expect](https://www.rfc-editor.org/rfc/rfc9110.html#field.expect) has been restored for compatibility with RFC 2616. ([Section 10.1.1](https://www.rfc-editor.org/rfc/rfc9110.html#field.expect))
...
"Accept Parameters" (accept-params and accept-ext ABNF production) have been removed from the definition of the Accept field. ([Section 12.5.1](https://www.rfc-editor.org/rfc/rfc9110.html#field.accept))

The Accept-Charset field is now deprecated. ([Section 12.5.2](https://www.rfc-editor.org/rfc/rfc9110.html#field.accept-charset))

The semantics of "*" in the [Vary](https://www.rfc-editor.org/rfc/rfc9110.html#field.vary) header field when other values are present was clarified. ([Section 12.5.5](https://www.rfc-editor.org/rfc/rfc9110.html#field.vary))

Range units are compared in a case-insensitive fashion. ([Section 14.1](https://www.rfc-editor.org/rfc/rfc9110.html#range.units))

The use of the Accept-Ranges field is not restricted to origin servers. ([Section 14.3](https://www.rfc-editor.org/rfc/rfc9110.html#field.accept-ranges))

The process of creating a redirected request has been clarified. ([Section 15.4](https://www.rfc-editor.org/rfc/rfc9110.html#status.3xx))

Status code 308 (previously defined in [[RFC7538](https://www.rfc-editor.org/rfc/rfc9110.html#RFC7538)]) has been added so that it's defined closer to status codes 301, 302, and 307. ([Section 15.4.9](https://www.rfc-editor.org/rfc/rfc9110.html#status.308))
...
Status code 422 (previously defined in [Section 11.2](https://www.rfc-editor.org/rfc/rfc4918#section-11.2) of [[WEBDAV](https://www.rfc-editor.org/rfc/rfc9110.html#WEBDAV)]) has been added because of its general applicability. ([Section 15.5.21](https://www.rfc-editor.org/rfc/rfc9110.html#status.422))
...
The fact that If-Unmodified-Since does not apply to a resource without a concept of modification time has been clarified. ([Section 13.1.4](https://www.rfc-editor.org/rfc/rfc9110.html#field.if-unmodified-since))

Preconditions can now be evaluated before the request content is processed rather than waiting until the response would otherwise be successful. ([Section 13.2](https://www.rfc-editor.org/rfc/rfc9110.html#evaluation))
...
It is now possible to define Range handling on extension methods. ([Section 14.2](https://www.rfc-editor.org/rfc/rfc9110.html#field.range))

Described use of the [Content-Range](https://www.rfc-editor.org/rfc/rfc9110.html#field.content-range) header field ([Section 14.4](https://www.rfc-editor.org/rfc/rfc9110.html#field.content-range)) as a request modifier to perform a partial PUT. ([Section 14.5](https://www.rfc-editor.org/rfc/rfc9110.html#partial.PUT))
...
This specification includes the extension defined in [[RFC7694](https://www.rfc-editor.org/rfc/rfc9110.html#RFC7694)] but leaves out examples and deployment considerations.
