...
> 原文：[https://manned.org/man/ubuntu-noble/tar](https://manned.org/man/ubuntu-noble/tar)
...
DESCRIPTION
   ...
   traditional style, the first argument is a cluster of option letters and
...
In GNU or long-option style, each option begins with two dashes and has a
meaningful name, consisting of lower-case letters and dashes.  When used,
the long option can be abbreviated to its initial letters, provided that
this does not create ambiguity.  Arguments to long options are supplied
either as a separate command line word, immediately following the option,
or separated from the option by an equals sign with no intervening
...
old options is not encouraged.
   ...
   to perform.  Exactly one of them must be given.  The meaning of non-
   option arguments depends on the operation mode requested.
...
--test-label
      ...
      0 if a match is found, and with code 1 otherwise.  No output is
...
-u, --update
      ...
      options.  Notice, that newer files don't replace their old archive
      copies, but instead are appended to the end of archive.  The
      resulting archive can thus contain several members of the same
...
--level=NUMBER
      ...
      truncate the snapshot file before dumping, thereby forcing a level
      0 dump.
...
--occurrence[=N]
      ...
      option is valid only when used with one of the following
      subcommands: --delete, --diff, --extract or --list and when a list
      of files is given either on the command line or via the -T option.
      The default N is 1.
...
--sparse-version=MAJOR[.MINOR]
      ...
      implies --sparse.  Valid argument values are 0.0, 0.1, and 1.0.
...
-S, --sparse
      ...
      prior to archiving it, and if so, to reduce the resulting archive
...
--group=NAME[:GID]
      Force NAME as group for added files.  If GID is not supplied, NAME
      can be either a user name or numeric GID.  In this case the
...
--group-map=FILE
      ...
      group.  It must consist of two fields, delimited by any amount of
      whitespace:
...
--owner=NAME[:UID]
      Force NAME as owner for added files.  If UID is not supplied, NAME
      can be either a user name or numeric UID.  In this case the
...
--owner-map=FILE
      ...
      It must consist of two fields, delimited by any amount of
      whitespace:
...
--xattrs-exclude=PATTERN
      Specify the exclude pattern for xattr keys.  PATTERN is a globbing
      pattern, e.g. --xattrs-exclude='user.*' to include only attributes
      from the user namespace.
    
--xattrs-include=PATTERN
      Specify the include pattern for xattr keys.  PATTERN is a globbing
      pattern.
...
-F, --info-script=COMMAND, --new-volume-script=COMMAND
      Run COMMAND at the end of each tape (implies -M).  The command can
      include arguments.  When started, it will inherit tar's
...
-i, --ignore-zeros
      ...
      512-blocks filled with zeroes mean EOF and tar stops reading after
...
--no-verbatim-files-from
      ...
      This is the default behavior.  The --no-verbatim-files-from option
      is provided as a way to restore it after --verbatim-files-from
      option.
...
--verbatim-files-from
      ...
      --files-from (-T) option.  The default behavior is to handle names
      supplied in file lists as if they were typed in the command line,
      i.e. any names starting with a dash are treated as tar options.
      The --verbatim-files-from option disables this behavior.
...
--warning=KEYWORD
      ...
      allEnable all warning messages.  This is the default.
     ...
     A common example of what you can get when using this
     warning is:
...
-w, --interactive, --confirmation
  ...
  --no-same-owner.
   ...
   Size suffixes
   SuffixUnits   Byte Equivalent
   b Blocks  SIZE x 512
   B Kilobytes   SIZE x 1024
   c Bytes   SIZE
   G Gigabytes   SIZE x 1024ˆ3
   K Kilobytes   SIZE x 1024
   k Kilobytes   SIZE x 1024
   M Megabytes   SIZE x 1024ˆ2
   P Petabytes   SIZE x 1024ˆ5
   T Terabytes   SIZE x 1024ˆ4
   w Words   SIZE x 2
...
1  Some files differ.  If tar was invoked with the --compare (--diff,
      ...
      --create, --append or --update options, this exit code means that
      some files were changed while being archived and so the resulting
...
compressor program failed.  Another example is rmt failure during backup
to a remote device.
