+++
title = "ab"
date = 2024-10-11T19:40:24+08:00
weight = 0
type = "docs"
description = ""
isCJKLanguage = true
...
https://manpages.ubuntu.com/manpages/noble/en/man1/ab.1.html
...
```
       ...
       protocol ] [ -g gnuplot-file ] [ -h ] [ -H custom-header ] [ -i ] [ -k ] [ -l ] [ -m HTTP-
       method ] [ -n requests ] [ -p POST-file ] [ -P proxy-auth-username:password ] [ -q ] [  -r
       ]  [  -s  timeout  ]  [  -S  ]  [  -t timelimit ] [ -T content-type ] [ -u PUT-file ] [ -v
       verbosity] [  -V  ]  [  -w  ]  [  -x  <table>-attributes  ]  [  -X  proxy[:port]  ]  [  -y
...
```
       ...
       -E client-certificate-file
              When connecting to an SSL website, use  the  provided  client  certificate  in  PEM
              format  to authenticate with the server. The file is expected to contain the client
