...
```
   ...
   Logging and Input File Options
       ...
       --metalink-index=number
           ...
           good one.  Metaurls, such as those from a --metalink-over-http, may have been sorted
   ...
   Download Options
       ...
       --dns-servers=ADDRESSES
           ...
           e.g. as configured in /etc/resolv.conf.  ADDRESSES may be specified either as IPv4 or
           IPv6 addresses, comma-separated.  Wget needs to be built with libcares for this option
           to be available.
       ...
       --progress=type
           ...
           the scrolling filename in the progress bar.  By passing the "noscroll" parameter, Wget
           can be forced to display as much of the filename as possible without scrolling through
           it.
       ...
       --show-progress
           ...
           verbosity modes like --no-verbose or --quiet.  This is often a desired a property when
           invoking wget to download several small/large files.  In such a case, wget could
       ...
       --quota=quota
           ...
           Note that quota will never affect downloading a single file.  So if you specify wgetNAME
...
DESCRIPTION
   ...
   Option styles
       ...
       the option letters.  Any command line words  that  remain  after  all  options  have  been
       processed are treated as non-option arguments: file or archive member names.
       ...
       other command line utilities.  If an option takes an argument, the  argument  follows  it,
   ...
   Operation mode
       ...
       Exactly one of them must be given.  The meaning of non-option  arguments  depends  on  the
       operation mode requested.

       -A, --catenate, --concatenate
              Append  archives  to  the end of another archive.  The arguments are treated as the
              ...
              the resulting archive only when using the -i (--ignore-zeros) option.
       ...
       -u, --update
              ...
              have the same meaning as with the -c and -r  options.   Notice,  that  newer  files
              don't  replace  their  old  archive  copies, but instead are appended to the end of
              archive.  The resulting archive can thus contain several members of the same  name,
...
OPTIONS
   Operation modifiers
       ...
       -g, --listed-incremental=FILE
              ...
              archives  of  non-zero  level  N,  you need a copy of the snapshot file created for
              level N-1, and use it as FILE.

              When listing or extracting, the actual content of FILE  is  not  inspected,  it  is
       ...
       --level=NUMBER
              Set dump level for a created listed-incremental archive.  Currently only  --level=0
              is  meaningful:  it  instructs  tar  to  truncate the snapshot file before dumping,
       ...
       --sparse-version=MAJOR[.MINOR]
              Set which version of the sparse format  to  use.   This  option  implies  --sparse.
              Valid  argument  values are 0.0, 0.1, and 1.0.  For a detailed discussion of sparse
              formats, refer to the GNU Tar Manual, appendix D, "Sparse Formats".  Using the info
              reader,  it  can  be  accessed  running  the  following  command:  info tar 'Sparse
              Formats'.
   ...
   Output stream selection
       ...
       --to-command=COMMAND
              ...
              the contents of the file being extracted supplied to  it  on  its  standard  input.
   ...
   Handling of file attributes
       ...
       -p, --preserve-permissions, --same-permissions
              Set permissions of extracted files to those recorded in the  archive  (default  for
              superuser).
       ...
       -s, --preserve-order, --same-order
              Tell tar that the list of file names to process is sorted in the same order as  the
   ...
   Informative output
       ...
       -v, --verbose
              ...
              increases the verbosity level by one.  The maximum verbosity level  is  3.   For  a
              detailed  discussion  of  how  various verbosity levels affect tar's output, please
              refer to GNU Tar Manual, subsection 2.5.2 "The '--verbose' Option".
...
RETURN VALUE
       Tar's exit code indicates whether it  was  able  to  successfully  perform  the  requested
...
COPYRIGHT
       ...
       --prefer-family=none/IPv4/IPv6
           ...
           to 2001:200:0:8002:203:47ff:fea5:3085 and to 203.178.141.194.  When the preferred
       ...
       --use-askpass=command
           ...
           specified then the command in the environment variable WGET_ASKPASS is used.  If
           WGET_ASKPASS is not set then the command in the environment variable SSH_ASKPASS is
           used.
   ...
   HTTP Options
       ...
       --adjust-extension
           ...
           downloading CGI-generated materials.  A URL like http://site.com/article.cgi?25 will
       ...
       --http-password=password
           ...
           from being seen, use the --use-askpass or store them in .wgetrc or .netrc, and make
           sure to protect those files from other users with "chmod".  If the passwords are
           really important, do not leave them lying in those files either---edit the files and
       ...
       --post-file=file
           ...
           This example shows how to log in to a server using POST and then proceed to download
       ...
       --trust-server-names
           ...
           URL.  By default the local file name is based on the original URL.  When doing
           recursive retrieving this can be helpful because in many web sites redirected URLs
       ...
       --retry-on-http-error=code[,code,...]
           ...
           separated list of 3-digit HTTP response codes as argument. Useful to work around
   ...
   HTTPS (SSL/TLS) Options
       ...
       --crl-file=file
           Specifies a CRL file in file.  This is needed for certificates that have been
           revocated by the CAs.
       ...
       --hsts-file=file
           ...
           is discouraged by RFC 6797 (see Appendix B "Differences between HSTS Policy and Same-
           Origin Policy"). Thus, this functionality should not be used in production
           ...
           more information about the potential security threats arose from such practice, see
   ...
   FTP Options
       ...
       --retr-symlinks
           ...
           When --retr-symlinks=no is specified, the linked-to file is not downloaded.  Instead,
           a matching symbolic link is created on the local file system.  The pointed-to file
           will not be retrieved unless this recursive retrieval would have encountered it
   ...
   Recursive Accept/Reject Options
       ...
       --regex-type regextype
           Specify the regular expression type.  Possible types are posix or pcre.  Note that to
           be able to use pcre type, wget has to be compiled with libpcre support.
...
```
       ...
       no_proxy
           ...
           not be used for.  For instance, if the value of no_proxy is .mit.edu, proxy will not
...
```
       ...
       2.  Try to repeat the bug in as simple circumstances as possible.  E.g. if Wget crashes
           while downloading wget -rl0 -kKE -t5 --no-proxy http://example.com -o /tmp/log, you
       ...
       3.  Please start Wget with -d option and send us the resulting output (or relevant parts
           ...
           data.  Since the bug address is publicly archived, you may assume that all bug reports
